...
namespace main_savitch_14
{
	...
	while (!is_game_over( )) // un-comment this
	{
		...
		{
			make_computer_move( );
			cout << "It's white's turn to place a piece." << endl;
			cout << endl;
		}
	}
    ...
    string game::get_user_move( ) const
    {
    	...
    	else
    	{
			...
			while(!move5.empty())
			{
				string temp = move5.front();
				cout << temp << " ";
				move5.pop();
			}
			cout << endl;
    	    cout << "It's black's turn to place a piece." << endl;;
    	}
		display_message("Your move, please (Ex: A1. Use XX to skip a turn. Skip twice to end game, if no possible moves): ");
		getline(cin, answer);
		return answer;
    }
    ...
    void game::make_human_move( )
    {
		...
		while (!is_legal(move))
		{
			cout << "Possible moves available." << endl;
